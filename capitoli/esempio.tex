...
Il secondo scenario mostra il meccanismo opposto, quello dell'impostazione assente. Un social network espone i messaggi privati tramite una chiamata a \path{/dm/user_updates.json} con identificativo di conversazione e cursore; poiché la risposta non include l'intestazione \texttt{Cache-Control}, le conversazioni private finiscono nella cache del browser, da cui possono essere recuperate leggendo i file sul filesystem~\cite{owasp2023api8}. Qui non c'è nulla di attivo da disabilitare: c'è una direttiva che nessuno ha inviato, e la conseguenza è che dati riservati vengono conservati in chiaro su una macchina che l'applicazione non controlla.
