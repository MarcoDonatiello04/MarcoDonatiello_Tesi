...
Il controllo funzionale non manca per una svista isolata: manca perché il luogo
in cui dovrebbe stare non è uno solo. OWASP osserva che l'autorizzazione su una
funzione o una risorsa è gestita di norma tramite configurazione oppure a
livello di codice, e che implementare controlli corretti può risultare un
compito confuso, dato che le applicazioni moderne contengono molti tipi di
ruoli, gruppi e gerarchie di utenti complesse, con sotto-utenti e utenti
titolari di più di un ruolo~\cite{owasp2023api5}. La difficoltà non sta nello
scrivere il singolo confronto, che è banale, ma nel sapere quale confronto
scrivere e nel ricordarsene in ogni punto in cui una funzione riservata viene
esposta.

A questa dispersione si somma una scelta di progetto che OWASP rende esplicita
solo dal lato della contromisura, prescrivendo un meccanismo che neghi ogni
accesso per impostazione predefinita e richieda concessioni esplicite ai
singoli ruoli per ogni funzione~\cite{owasp2023api5}. La prescrizione descrive
in negativo ciò che si osserva nella pratica: il valore predefinito è
permissivo, e una rotta aggiunta a un controller esistente eredita il routing e
l'autenticazione, non l'autorizzazione. La funzione nasce quindi raggiungibile,
e resta tale finché qualcuno non decide che non dovrebbe esserlo.

Lo sfruttamento, di conseguenza, non richiede alcuna manipolazione. OWASP
osserva che è sufficiente inviare chiamate legittime a un endpoint cui non si
dovrebbe avere accesso, e che questi difetti sono più facili da scoprire nelle
API perché le API sono più strutturate e il modo di raggiungere certe funzioni
è più prevedibile, come nella sostituzione del metodo \texttt{GET} con
\texttt{PUT} o della stringa \texttt{users} con \texttt{admins}
nell'URL~\cite{owasp2023api5}. È un'osservazione che va letta per intero: la
stessa regolarità che rende un'API documentabile e prevedibile per un client
legittimo la rende enumerabile per chiunque altro, e ciò che nel web
tradizionale richiedeva di indovinare il nome di una pagina qui si riduce a
percorrere le poche varianti che una convenzione ammette.

Il primo scenario del documento mostra il meccanismo nella sua forma più
istruttiva. In un'applicazione ad accesso su invito, l'app mobile invoca
\texttt{GET /api/invites/\{invite\_guid\}} e riceve un JSON con i dettagli
dell'invito, fra cui il ruolo e l'indirizzo di posta dell'utente. L'attaccante
duplica la richiesta, ne modifica metodo ed endpoint in \texttt{POST
/api/invites/new} --- un endpoint che dovrebbe essere invocato soltanto dagli
amministratori attraverso la console di amministrazione, la quale non
implementa alcuna autorizzazione a livello di funzione --- e si invia un invito
con ruolo \texttt{admin}, ottenendo un \texttt{201 Created}~\cite{owasp2023api5}.
Due dettagli meritano di essere isolati. Il primo è che il controllo, in un
certo senso, esisteva: risiedeva nella console, cioè nel fatto che soltanto un
amministratore vedesse quel pulsante. L'applicazione ha usato la
raggiungibilità dell'interfaccia come sostituto del diritto, e la sostituzione
regge finché il client è l'unico modo di parlare con l'API. Il secondo è che la
lettura precedente ha fornito la forma del corpo da inviare, mostrando
l'esistenza del campo \texttt{role}: l'endpoint di lettura non era vulnerabile,
ma ha reso banale la costruzione della richiesta di scrittura. Il secondo
scenario è più diretto e vale come promemoria sul valore dell'oscurità: un
endpoint \texttt{GET /api/admin/v1/users/all}, che restituisce i dettagli di
tutti gli utenti e non implementa controlli a livello di funzione, viene
raggiunto da un attaccante che, avendo appreso la struttura dell'API, tira a
indovinare~\cite{owasp2023api5}.

Una raccomandazione del documento chiude il quadro delle cause e conviene
riportarla per esteso, perché smentisce l'euristica più diffusa: non si deve
presumere che un endpoint sia ordinario o amministrativo sulla sola base del
percorso, poiché, per quanto gli sviluppatori tendano a esporre la maggior
parte degli endpoint amministrativi sotto un percorso dedicato come
\texttt{api/admins}, è molto comune trovarli sotto altri percorsi insieme a
quelli ordinari, per esempio \texttt{api/users}~\cite{owasp2023api5}. La
funzione amministrativa, in altri termini, non è tipograficamente
riconoscibile.

Restano due fattori che spostano il difetto fuori dal codice applicativo. Le
configurazioni che lasciano abilitati verbi HTTP non necessari riaprono
un'operazione attraverso un metodo che nessuno ha pensato di
proteggere~\cite{owasp2023api8}, ed è esattamente la mossa del primo scenario;
le versioni obsolete e gli host dimenticati continuano a servire il gestore
precedente al controllo, come mostra lo scenario in cui una vecchia versione
esposta consente di cambiare la password di qualunque
utente~\cite{owasp2023api9}. In entrambi i casi il controllo è stato scritto,
ma non copre tutta la superficie su cui la funzione è raggiungibile.

\paragraph{Impatto essenziale}

OWASP colloca l'impatto tecnico nella fascia grave e ne descrive la
progressione: difetti di questo tipo consentono all'attaccante di accedere a
funzionalità non autorizzate, le funzioni amministrative sono il bersaglio
privilegiato, e l'esito può essere la divulgazione, la perdita o la corruzione
dei dati, fino all'interruzione del servizio~\cite{owasp2023api5}.

L'elemento che distingue questa categoria è che il danno non scala con il
numero di identificatori ma con la potenza della funzione raggiunta. Una BOLA
produce un record per richiesta, e ottenere l'intera base dati richiede
un'enumerazione che lascia nel traffico una traccia riconoscibile; una BFLA su
un endpoint di esportazione produce lo stesso risultato con una chiamata sola,
indistinguibile per volume da qualunque altra. La rumorosità dell'attacco è
inversamente proporzionale al privilegio della funzione che lo consente.

La seconda differenza riguarda la durata. Il primo scenario non si conclude con
la lettura di un dato: si conclude con un account amministrativo creato
attraverso il flusso legittimo dell'applicazione~\cite{owasp2023api5}. La
vulnerabilità, cioè, converte un accesso occasionale in un'identità stabile, e
da quel momento ogni azione successiva è a tutti gli effetti l'azione di un
amministratore: autenticata, autorizzata, indistinguibile nei registri. Chiudere
il difetto in un secondo momento non revoca l'account, e la bonifica smette di
essere una modifica al codice per diventare una verifica sull'inventario delle
utenze.

Vale infine osservare che le tre proprietà di sicurezza cadono qui insieme,
perché le funzioni riservate sono in prevalenza funzioni di scrittura. Lo
stesso difetto che espone l'esportazione degli utenti espone, sull'endpoint
accanto, la loro cancellazione, e l'interruzione di servizio che OWASP elenca
come esito ultimo non nasce dall'esaurimento di una risorsa, come in
API4:2023, ma da una funzione distruttiva perfettamente funzionante invocata da
chi non doveva poterla invocare.

\paragraph{Mitigazione}

La prescrizione centrale è una sola e riguarda il valore predefinito: il
meccanismo di applicazione deve negare ogni accesso per impostazione
predefinita, richiedendo concessioni esplicite ai ruoli specifici per l'accesso
a ogni funzione~\cite{owasp2023api5}. L'inversione conta più di quanto la sua
formulazione lasci intendere, perché sposta il costo dell'errore: con il
diniego predefinito una dimenticanza produce un \texttt{403} che emerge al
primo test funzionale, con il permesso predefinito produce silenzio, e il
silenzio non è un sintomo che qualcuno vada a cercare.

Su questo impianto il documento innesta una revisione degli endpoint condotta
tenendo presenti la logica di business dell'applicazione e la gerarchia dei
gruppi~\cite{owasp2023api5}: nessuna scorciatoia, quindi, rispetto alla
conoscenza del dominio, ed è coerente con l'osservazione che il percorso non
dice se una funzione sia amministrativa.

La terza raccomandazione è l'unica ad avere una forma sintattica, e per questo
è anche la più interessante: i controller amministrativi devono ereditare da un
controller astratto amministrativo che implementi i controlli di autorizzazione
in base al gruppo o al ruolo dell'utente, e le funzioni amministrative
collocate dentro un controller ordinario devono implementare il controllo in
base al ruolo~\cite{owasp2023api5}. La misura non aggiunge verifiche, le
riposiziona: il controllo diventa una proprietà della gerarchia delle classi
anziché del singolo gestore, e ometterlo richiede l'atto positivo di collocare
un gestore fuori da quella gerarchia. È una differenza sostanziale sia per la
revisione umana, cui l'anomalia si presenta come una discontinuità strutturale,
sia per l'analisi automatica, che acquisisce finalmente un predicato
verificabile senza conoscere le intenzioni di chi ha scritto il codice.

Resta il problema della copertura, che nessuna delle tre misure risolve. Un
controllo corretto protegge soltanto la superficie effettivamente servita dal
codice che lo contiene: le versioni deprecate e gli host ancora esposti
continuano a servire il gestore privo di
verifica~\cite{owasp2023api9}, e le funzionalità non necessarie lasciate
attive, verbi HTTP compresi, offrono all'operazione una via che il controllo
non presidia~\cite{owasp2023api8}. Va inoltre segnalata una differenza rispetto
ad API4:2023, dove la delega dell'onere a un gateway è una scelta
architetturale legittima. Qui la delega può essere solo parziale: un gateway
può negare un prefisso di percorso, ma la gerarchia di ruoli, gruppi,
sotto-utenti e utenti con più ruoli che OWASP descrive~\cite{owasp2023api5}
vive nel modello di dominio dell'applicazione, e la decisione che vi si fonda
non può essere presa altrove.

\paragraph{Rilevabilità: stato dell'arte in letteratura}

OWASP classifica la rilevabilità come facile e ne dà la ragione: le API sono
strutturate e il modo di raggiungere certe funzioni è prevedibile, al punto che
sostituire un verbo o un segmento del percorso basta a produrre il
tentativo~\cite{owasp2023api5}. Il giudizio è corretto, ma riguarda una sola
metà del problema. Ciò che è facile è formulare la chiamata candidata; ciò che
resta difficile è stabilire se la risposta ottenuta fosse dovuta, perché un
\texttt{200 OK} attesta che l'operazione è stata eseguita, non che fosse
legittimo eseguirla. Il verdetto richiede una nozione di ciò che il chiamante
avrebbe dovuto poter fare, e quella nozione non risiede nel sistema osservato.

Che la scorciatoia sintattica non basti lo dichiara la fonte primaria stessa,
quando avverte di non dedurre dal percorso la natura amministrativa di un
endpoint~\cite{owasp2023api5}. Una regola che segnali i gestori esposti sotto
\texttt{api/admins} e privi di verifica produce quindi falsi negativi per
costruzione, e li produce proprio nei casi che il documento indica come i più
comuni.

È da questa lacuna che muove la letteratura sul rilevamento, il cui contributo
principale non è l'ispezione del codice ma la ricostruzione della specifica
mancante. Se ne riconoscono quattro strategie. La prima ricava la specifica
dalla navigazione: Sun, Xu e Su derivano dalle pagine raggiungibili da ciascun
ruolo le regole di controllo di accesso implicite nell'applicazione, e
verificano poi staticamente quali percorsi consentano di aggirarle~\cite{sun2011static}.
La seconda la ricava dalla forma del codice: RoleCast parte da
un'osservazione di ingegneria del software, e cioè che le applicazioni web sono
partizionate per ruolo e che all'interno di una partizione i controlli sono
applicati in modo coerente, e da quella coerenza inferisce quali condizioni
siano controlli di sicurezza --- senza sapere in anticipo che aspetto abbiano,
come il titolo dichiara --- segnalando i percorsi in cui
mancano~\cite{son2011rolecast}. La terza ragiona sul contesto di
autorizzazione: MACE affronta l'escalation nelle due direzioni distinte più
sopra, verticale e orizzontale, su applicazioni di dimensioni
reali~\cite{monshizadeh2014mace}. La quarta rinuncia a inferire e sceglie di
scrivere: Atlidakis, Godefroid e Polishchuk definiscono un insieme di regole di
sicurezza per le API REST e le verificano con un fuzzing stateful guidato dalla
descrizione dell'interfaccia~\cite{atlidakis2020checking}, dove la descrizione
fornisce la grammatica delle richieste e le regole l'oracolo che il codice non
contiene. Sul versante black box si colloca il lavoro più recente, che affronta
il rilevamento automatico dei difetti di controllo di accesso osservando
l'applicazione dall'esterno~\cite{liu2025bacscan}.

Il confronto con API4:2023 chiarisce di quale incertezza si tratti, perché le
due categorie condividono il sintomo --- lo strumento non trova un controllo ---
e non la causa. Per API4 l'assenza è locale e potenzialmente irrilevante,
giacché il limite può risiedere nel gateway o nella piattaforma: lo strumento
non sa dove guardare. Per API5 l'assenza è invece autentica, dato che la
decisione sul ruolo non può essere presa fuori dall'applicazione, ma manca la
norma rispetto a cui quell'assenza costituisce un difetto: lo strumento non sa
che cosa cercare. È la ragione per cui ciascuno dei lavori citati dedica la
parte più consistente del proprio contributo a costruire una specifica, e
soltanto la parte residua a verificarla.

La conseguenza operativa è che l'unico oracolo che non richieda di leggere le
intenzioni di chi ha progettato il sistema è comparativo: la stessa chiamata,
due account di privilegio diverso, esiti che divergono. Il differenziale
sostituisce la norma assente con un'osservazione, e proprio per questo la
facilità dichiarata da OWASP presuppone in silenzio una condizione non sempre
disponibile, ossia il possesso di credenziali su almeno due livelli della
gerarchia. La sonda, inoltre, è invasiva per natura, perché le chiamate da
mettere alla prova sono esattamente quelle che creano, modificano e cancellano.
Ne discende la divisione dei compiti adottata nel seguito: il segnale statico
--- un gestore che espone un'operazione riservata senza che alcun controllo di
ruolo lo protegga, o che vive fuori dalla gerarchia amministrativa raccomandata
da OWASP~\cite{owasp2023api5} --- serve a selezionare i pochi endpoint su cui
vale la pena esporsi, mentre il verdetto spetta alla verifica differenziale su
quel sottoinsieme ridotto. Preso da solo, il segnale statico resta un'ipotesi:
dice che un controllo non c'è, non che sarebbe dovuto esserci.
...
\paragraph{Impatto essenziale}
OWASP elenca come esiti di uno sfruttamento riuscito l'enumerazione dei servizi interni, tipicamente attraverso scansione di porte, la divulgazione di informazioni, l'aggiramento di firewall o altri meccanismi di sicurezza, e in alcuni casi il denial of service o l'uso del server come proxy per occultare attività malevole~\cite{owasp2023api7}. CWE-918 organizza gli stessi effetti sulle tre proprietà: lettura di dati applicativi sulla riservatezza, esecuzione di codice o comandi non autorizzati sull'integrità, aggiramento dei meccanismi di protezione sul controllo di accesso~\cite{cwe918}.
...
Va infine registrata un'ammissione che OWASP fa esplicitamente e che nessun'altra categoria contiene: il rischio SSRF non può sempre essere eliminato del tutto, e nella scelta del meccanismo di protezione occorre considerare i rischi e le esigenze di business~\cite{owasp2023api7}. La ragione è che la funzionalità e la vulnerabilità coincidono. Un servizio che offre anteprime di collegamenti arbitrari deve, per definizione, effettuare richieste verso indirizzi che l'utente sceglie; ciò che si può fare è confinare l'effetto di quelle richieste, non impedirle.

\paragraph{Rilevabilità: stato dell'arte in letteratura}
Per la prima volta nel corso di questo capitolo le due fonti primarie concordano, e concordano nel senso favorevole all'analisi statica. OWASP classifica la rilevabilità come facile, precisando però che l'analisi delle richieste e delle risposte è necessaria per individuare il problema e che quando la risposta non viene restituita, nella forma cieca, il rilevamento richiede maggiore sforzo e creatività~\cite{owasp2023api7}. CWE-918 è ancora più netta: la sezione dei metodi di rilevamento contiene una sola voce, l'analisi statica automatica, descritta come costruzione di un modello di flusso dei dati e di controllo seguita dalla ricerca di pattern potenzialmente vulnerabili che connettono sorgenti, ossia le origini dell'input, a sink, ossia le destinazioni dove il dato interagisce con componenti esterni, e la sua efficacia è dichiarata alta~\cite{cwe918}.
...
Con API4:2023 il contatto è sui limiti dichiarativi. I vincoli su memoria, CPU e processi che OWASP raccomanda di affidare a container e funzioni serverless~\cite{owasp2023api4} vivono in file di orchestrazione, e la loro assenza è tanto un difetto di consumo di risorse quanto una configurazione carente nel senso di API8, che chiede espressamente di rivedere i file di orchestrazione e i servizi cloud~\cite{owasp2023api8}.

Con API5:2023 il contatto è sul metodo HTTP, già osservato in quella sede. La raccomandazione di API8 di specificare quali verbi siano ammessi per ciascuna API e di disabilitare tutti gli altri~\cite{owasp2023api8} previene per via configurativa il caso in cui una rotta risulti protetta in lettura e scoperta in cancellazione. La differenza sta nel livello a cui il rimedio agisce: nell'applicazione per API5, nella catena dei server per API8.

Con API7:2023 il contatto riguarda la politica di uscita. Lo scenario che OWASP porta per API8 lo mostra con chiarezza, poiché l'esecuzione di codice remoto vi diventa possibile per la combinazione di una configurazione predefinita insicura dell'utilità di logging e di una politica di rete in uscita permissiva~\cite{owasp2023api8}: la seconda condizione è esattamente quella che rende sfruttabile anche l'SSRF. La Tabella~\ref{tab:api8_relazioni} riassume.
...
Il primo scenario di OWASP illustra il meccanismo dei valori predefiniti pericolosi, e lo fa con quello che è riconoscibilmente il caso Log4Shell. Un server di back-end tiene un registro degli accessi scritto da una diffusa utilità di logging open source che supporta l'espansione di segnaposto e le ricerche JNDI, entrambe abilitate per impostazione predefinita; per ogni richiesta viene scritta una riga secondo un formato che include il percorso e il codice di stato. L'attaccante invia una richiesta apparentemente innocua verso \texttt{/health} con un'intestazione \texttt{X-Api-Version} contenente un riferimento JNDI a un server LDAP che controlla, e nel momento in cui l'utilità espande quel valore per scriverlo nel registro scarica ed esegue l'oggetto malevolo dal server remoto~\cite{owasp2023api8}. Vale la pena osservare cosa manca in questa catena: non c'è alcun difetto nel codice dell'applicazione, non c'è validazione dell'input che avrebbe potuto essere presente, e l'endpoint colpito è quello di verifica dello stato di salute, tipicamente il più innocuo dell'intera API.

Il secondo scenario mostra il meccanismo opposto, quello dell'impostazione assente. Un social network espone i messaggi privati tramite una chiamata a \texttt{/dm/user\_updates.json} con identificativo di conversazione e cursore; poiché la risposta non include l'intestazione \texttt{Cache-Control}, le conversazioni private finiscono nella cache del browser, da cui possono essere recuperate leggendo i file sul filesystem~\cite{owasp2023api8}. Qui non c'è nulla di attivo da disabilitare: c'è una direttiva che nessuno ha inviato, e la conseguenza è che dati riservati vengono conservati in chiaro su una macchina che l'applicazione non controlla.

Un terzo meccanismo, che gli scenari non illustrano ma che l'elenco degli indicatori nomina, riguarda le discrepanze nell'elaborazione delle richieste lungo la catena dei server~\cite{owasp2023api8}. È il fenomeno che la voce CWE-444 chiama interpretazione incoerente delle richieste HTTP e che la pratica conosce come \emph{request smuggling}: quando un bilanciatore di carico e un server applicativo delimitano diversamente il confine fra due richieste consecutive sulla stessa connessione, un attaccante può far sì che parte del proprio traffico venga attribuita alla richiesta di un altro utente. Il difetto non risiede in nessuno dei due componenti presi singolarmente, ma nella loro combinazione, ed è la ragione per cui OWASP raccomanda di assicurarsi che tutti i server della catena elaborino le richieste in maniera uniforme~\cite{owasp2023api8}.

Resta la causa più banale e più diffusa, quella che l'elenco degli indicatori mette al secondo posto: patch mancanti e sistemi non aggiornati~\cite{owasp2023api8}. Non è un errore di configurazione in senso stretto ma di manutenzione, e la sua presenza nella stessa categoria segnala che il confine fra le due cose, nella pratica operativa, è poco netto.

\paragraph{Impatto essenziale}
La formulazione di OWASP è breve e va letta nei suoi due termini: le configurazioni insicure non solo espongono dati sensibili degli utenti, ma anche dettagli del sistema che possono condurre alla compromissione completa del server~\cite{owasp2023api8}. La seconda parte è quella che giustifica la classificazione dell'impatto tecnico come severo, e i due scenari ne mostrano i due estremi: nel secondo il danno è l'esposizione di conversazioni private, nel primo è l'esecuzione di codice arbitrario sul server.
...
\paragraph{Definizione e collocazione}
Le nove categorie esaminate finora hanno un tratto comune che diventa visibile solo quando si incontra la decima: descrivono tutte l'API come bersaglio di un attacco diretto. API10:2023 rovescia la prospettiva. Qui l'attaccante non colpisce l'API bersaglio ma un servizio da cui essa dipende, e il danno arriva attraverso un canale che l'applicazione considera fidato. Le note di rilascio dell'edizione 2023 dichiarano di aver introdotto la categoria per affrontare un fenomeno osservato di recente, ossia il fatto che gli attaccanti hanno cominciato a cercare i servizi integrati di un bersaglio per compromettere quelli, anziché colpire direttamente le API del bersaglio stesso~\cite{owasp2023apireleasenotes}.

La causa che OWASP individua è di ordine psicologico prima che tecnico, e il documento la enuncia senza mediazioni: gli sviluppatori tendono a fidarsi dei dati ricevuti da API di terze parti più di quanto si fidino dell'input dell'utente, e questo vale in particolare per le API offerte da aziende note; di conseguenza tendono ad adottare standard di sicurezza più deboli, per esempio in materia di validazione e sanificazione dell'input~\cite{owasp2023api10}. La stessa osservazione ritorna nella descrizione della debolezza, dove si legge che gli sviluppatori tendono a fidarsi senza verificare degli endpoint che interagiscono con API esterne o di terze parti, affidandosi a requisiti più deboli riguardo alla sicurezza del trasporto, all'autenticazione e autorizzazione, e alla validazione e sanificazione dell'input~\cite{owasp2023api10}.

Il documento elenca cinque condizioni che rendono l'API vulnerabile: interagire con altre API su un canale non cifrato; non validare e sanificare adeguatamente i dati raccolti da altre API prima di elaborarli o di passarli a componenti a valle; seguire ciecamente le redirezioni; non limitare la quantità di risorse disponibili per elaborare le risposte dei servizi di terze parti; non implementare timeout per le interazioni con essi~\cite{owasp2023api10}.

Su questo elenco vale la pena registrare un'obiezione sollevata pubblicamente sul repository del progetto durante la fase di revisione, secondo cui almeno tre di quelle voci apparterrebbero concettualmente ad altre categorie: il limite alle risorse per elaborare le risposte ricade nel consumo incontrollato di risorse, mentre l'interazione su canale non cifrato ricade nella configurazione insicura~\cite{owasp2023apiissue80}. L'osservazione è fondata e il gruppo di lavoro non l'ha recepita, sicché l'elenco pubblicato mescola condizioni che appartengono al nucleo della categoria, come la mancata validazione dei dati ricevuti e le redirezioni seguite ciecamente, con altre che sono la manifestazione di difetti già trattati altrove nella direzione del traffico in uscita.

I riferimenti esterni indicati sono tre — CWE-20 sulla validazione impropria dell'input, CWE-200 sull'esposizione di informazioni sensibili a un attore non autorizzato, CWE-319 sulla trasmissione in chiaro~\cite{owasp2023api10} — e la loro genericità riflette una proprietà reale della categoria anziché una scelta approssimativa. OWASP scrive infatti che l'impatto varia a seconda di ciò che l'API bersaglio fa con i dati che ha prelevato~\cite{owasp2023api10}: la debolezza specifica dipende dalla destinazione del dato non validato, che può essere una query SQL, un motore di template o un componente a valle qualsiasi. API10 individua quindi un punto d'ingresso, non un difetto terminale.

Le metriche assegnate presentano una tensione interna che merita di essere segnalata. La sfruttabilità è dichiarata facile, ma la descrizione del vettore afferma che lo sfruttamento richiede all'attaccante di identificare e potenzialmente compromettere altre API o servizi con cui l'API bersaglio è integrata, aggiungendo che di norma tale informazione non è pubblicamente disponibile o il servizio integrato non è facilmente attaccabile~\cite{owasp2023api10}. La prevalenza è comune, la rilevabilità media e l'impatto tecnico severo~\cite{owasp2023api10}. Se ne ricava che il giudizio di facilità riguarda la fase finale dell'attacco, quella in cui il dato malevolo viene consegnato e accettato senza controlli, mentre la fase preparatoria resta la parte costosa.

Il rapporto con le categorie contigue è definito dalla direzione del traffico e dalla natura della fiducia mal riposta.

Con API7:2023 la simmetria è quella già osservata in quella sede: entrambe riguardano richieste che il server emette, ma in API7 la destinazione è scelta dall'attaccante attraverso un input non validato, mentre in API10 la destinazione è legittima e il problema riguarda ciò che torna indietro. Le due categorie condividono la contromisura del divieto di seguire redirezioni automatiche~\cite{owasp2023api7,owasp2023api10}.
...
\paragraph{Mitigazione}
Le contromisure che OWASP elenca sono quattro e coprono piani distinti~\cite{owasp2023api10}.
...
\paragraph{Rilevabilità: stato dell'arte in letteratura}
OWASP classifica la rilevabilità come media e non fornisce, a differenza di altre categorie, indicazioni su come procedere; la descrizione della debolezza si limita a osservare che gli attaccanti devono identificare i servizi con cui il bersaglio si integra, cioè le sorgenti dei dati, ed eventualmente comprometterli~\cite{owasp2023api10}. L'assenza di una metodologia di verifica è essa stessa un'informazione, e conviene ricostruire il motivo.
