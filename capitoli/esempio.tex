...
algoritmico di una singola richiesta~\cite{crosby2003algorithmic}, di cui il ReDoS è
l'istanza più diffusa nei server a thread singolo~\cite{staicu2018redos}.
...
servizio~\cite{nist2019sp800204}: scelta corretta purché ogni host esposto stia
dietro quel gateway, mentre il limite scritto nel codice viaggia con
l'applicazione in ogni ambiente.

\paragraph{Rilevabilità}

OWASP giudica facile la rilevabilità di API4: richieste con parametri di
dimensione crescente e osservazione di stato, tempo e lunghezza della risposta
bastano di norma a far emergere il problema~\cite{owasp2023api4}. CWE-770
sostiene quasi l'opposto, e cioè che per le allocazioni prive di limite
l'analisi statica automatica offre un'utilità limitata~\cite{cwe770}. Il
contrasto è apparente. Le due fonti non parlano dello stesso osservatore: OWASP
descrive un tester in black box, CWE uno strumento che legge il codice
sorgente. Il test dinamico interroga il sistema nel suo insieme e riconosce il
limite ovunque sia stato applicato, ma vede soltanto l'host che interroga; un
ambiente beta esposto senza gateway resta fuori dal suo campo finché qualcuno
non lo inventaria. Sul versante statico la letteratura si è concentrata su
sottoclassi ben delimitate, il ReDoS~\cite{staicu2018redos} e il costo di
valutazione delle query GraphQL~\cite{cha2020graphql}. Per quanto è stato
possibile verificare, manca invece un corpo di lavori sottoposti a revisione
paritaria sul rilevamento statico dell'assenza di rate limiting, timeout o
limiti di paginazione nel codice di API REST. Ciò che esiste proviene dalla
pratica degli strumenti più che dalla ricerca: regole che ispezionano il
codice, i manifest infrastrutturali e il contratto OpenAPI. Di queste tre
direzioni solo le ultime due trovano appoggio in una fonte primaria,
nell'osservazione di CWE-770 sui file di configurazione che non fissano alcun
valore massimo~\cite{cwe770}.

La differenza rispetto alle categorie precedenti riguarda la natura stessa del
segnale statico. In una BOLA, o in un difetto di autenticazione, l'assenza del
controllo nel gestore coincide con la vulnerabilità, perché il controllo non
può risiedere altrove. Per API4 questa coincidenza si spezza. L'analisi del
codice non può affermare che il difetto esista, dato che il limite potrebbe
trovarsi nel gateway o nella piattaforma di orchestrazione; può affermare
soltanto che a un dato livello non c'è. È un'assenza locale, e come tale
diventa finding solo quando un secondo segnale --- un altro livello ispezionato,
una sonda dinamica --- la conferma. Lo scenario di API9 illustra il caso
simmetrico: il limite presente a un livello non protegge necessariamente tutti
gli host esposti~\cite{owasp2023api9}. Per questa categoria, dunque, la
correlazione fra segnali eterogenei non è un raffinamento dell'analisi ma la
